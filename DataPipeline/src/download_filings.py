...
def main():
    ...
    # If start_year is "current" or null/0, use current year
    if config.get("start_year") in [None, 0, "current", "auto"]:
        start_year = current_year
        LOGGER.info(f"Using current year as start_year: {start_year}")
    else:
        start_year = config["start_year"]
    
    # If end_year is "current" or null/0, use current year
    if config.get("end_year") in [None, 0, "current", "auto"]:
        end_year = current_year
        LOGGER.info(f"Using current year as end_year: {end_year}")
    else:
        end_year = config["end_year"]
    
    # Validation: ensure start_year <= end_year
    if start_year > end_year:
        LOGGER.error(f"start_year ({start_year}) cannot be greater than end_year ({end_year})")
        exit()
    
    LOGGER.info(f"Downloading filings for years: {start_year} to {end_year}")
    ...
    download_indices(
        start_year=start_year,  # Use dynamic year
        end_year=end_year,      # Use dynamic year
    ...
    tsv_filenames = []
    for year in range(start_year, end_year + 1):  # Use dynamic years
...
def get_specific_indices(
    ...
    Args:
            ...
            user_agent (str): The User-Agent string that will be declared to SEC EDGAR.
            cik_tickers (Optional[str]): Path to CSV file with CIKs/Tickers, or None to process all.
    ...
    # If cik_tickers path is provided
    if cik_tickers is not None:
        ...
        if isinstance(cik_tickers, str):
            if os.path.exists(cik_tickers) and os.path.isfile(cik_tickers):
                ...
                file_ext = os.path.splitext(cik_tickers)[1].lower()
                
                if file_ext == '.csv':
                    # Read CSV file with pandas
                    LOGGER.info(f"Reading companies from CSV: {cik_tickers}")
                    try:
                        companies_df = pd.read_csv(cik_tickers)
                        
                        # Check for CIK column (various possible names)
                        cik_column = None
                        for col in ['cik_int', 'CIK', 'cik', 'Cik']:
                        ...
                        # Extract CIKs and Tickers
                        cik_tickers_list = []
                        
                        if cik_column:
                            # Get CIKs (clean any commas from numbers like "1,045,810")
                            cik_values = companies_df[cik_column].dropna().astype(str)
                            cik_values = cik_values.str.replace(',', '').str.strip()
                            cik_tickers_list.extend(cik_values.tolist())
                            LOGGER.info(f"Found {len(cik_values)} CIKs in column '{cik_column}'")
                        
                        # if ticker_column:
                        #     # Get Tickers (filter out [NULL] values)
                        #     ticker_values = companies_df[ticker_column].dropna().astype(str)
                        #     ticker_values = ticker_values[ticker_values != '[NULL]']
                        #     ticker_values = ticker_values.str.strip()
                        #     cik_tickers_list.extend(ticker_values.tolist())
                        #     LOGGER.info(f"Found {len(ticker_values)} valid Tickers in column '{ticker_column}'")
                        
                        if not cik_tickers_list:
                            LOGGER.error("CSV file must contain 'cik_int' or 'ticker' column")
                            exit()
                        
                        # Remove duplicates while preserving order
                        cik_tickers_list = list(dict.fromkeys(cik_tickers_list))
                        LOGGER.info(f"Total unique companies to track: {len(cik_tickers_list)}")
                    ...
                    LOGGER.error(f"Unsupported file format: {file_ext}. Use .csv or .txt")
                    exit()
            else:
                # If it is not a valid filepath, log the error and exit
                LOGGER.error(f"File not found: {cik_tickers}")
                exit()

    # Check if cik_tickers_list exists and is not empty
    if 'cik_tickers_list' in locals() and len(cik_tickers_list):
        ...
        # Convert all tickers in the cik_tickers_list to CIKs
        for c_t in cik_tickers_list:
            # Clean the value (remove any whitespace, commas)
            c_t = str(c_t).strip().replace(',', '')
            
            if not c_t or c_t == '[NULL]':
                continue
                
            if c_t.isdigit():  # If it is a CIK
                ciks.append(str(int(c_t)))  # Convert to int then str to remove leading zeros
                LOGGER.debug(f"Added CIK: {c_t}")
                ...
                if c_t in ticker2cik:
                    ...
                    LOGGER.debug(f"Converted ticker {c_t} to CIK: {ticker2cik[c_t]}")
                else:
                    # If the ticker does not exist in the mapping, log the error
                    LOGGER.warning(f'Could not find CIK for ticker "{c_t}"')
        
        LOGGER.info(f"Successfully mapped {len(ciks)} companies to CIKs")
    ...
    for filepath in tsv_filenames:
        ...
        if len(ciks):
            # Normalize CIKs in dataframe (remove leading zeros for comparison)
            df['CIK_normalized'] = df['CIK'].astype(str).apply(lambda x: str(int(x)))
            df = df[df.CIK_normalized.isin(ciks)]
            df = df.drop(columns=['CIK_normalized'])
            LOGGER.info(f"Found {len(df)} filings for tracked companies in {os.path.basename(filepath)}")
    ...
    if len(dfs_list) == 0:
        LOGGER.warning("No filings found matching the criteria")
        return pd.DataFrame()
...
def crawl(
    ...
    if cik not in company_info_dict:
        ...
        company_info_soup = BeautifulSoup(request.content, "html.parser")
